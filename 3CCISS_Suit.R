...
ccissOutput <- function(SSPred,suit,rules,feasFlag){
 ### generate raw feasibility ratios
  ...
  suitVotes[,ModAgree := rowMaxs(as.matrix(.SD)), .SDcols = c("1","2","3","X")]
  ...
  suitVotes <- suitVotes %>% dplyr::select(SiteNo, Spp, FuturePeriod, SS_NoSpace, Curr, NewSuit, everything() )
  ...
  datFeas2 <- datFeas2 %>% mutate (Xadj2 = 1 - Xadj) %>% mutate(X2 = X + Xadj2)# %>% select (-Xadj)
   ## THEN VARIABLE THAT IS SUM OF COL1 + COL2*2, COL3*3, COL 'X'* 4 and then round(0) for establishment feasibility
  datFeas2 <- datFeas2 %>% mutate(NewSuit = `1` +(`2` * 2 ) + (`3` * 3) + (X2 * 4)) %>% mutate(across(NewSuit, round, 0))
  ...
  datFeas_final <- datFeas %>% dplyr::select(SiteNo,SS_NoSpace,Spp,Curr,NewSuit, SuitDiff, Flag) %>% mutate(across(NewSuit, round, 0))%>%
           mutate(NewSuit = sub('10', 'X', NewSuit)) 
  ...
  datEarly2 <- datEarly
  datEarly2 <- datEarly2 %>% mutate(Suit2025 = `1` +(`2` * 2 ) + (`3` * 3) + (X * 4)) %>% mutate (change2025 = Curr- Suit2025) %>% 
    mutate(across(Suit2025, round, 0)) %>% mutate(Suit2025 = sub('4', 'X', Suit2025))
  
  datEarly2 <- datEarly2 %>% mutate(Trajectory2025 = 
  ...
  datEarly2 <- datEarly2 %>% mutate(FailRisk2025 = 
                                  ifelse(X>.5, "High", 
  ...
  datMid2 <- datMid
  datMid2 <- datMid2 %>% mutate(Suit2055 = `1` +(`2` * 2 ) + (`3` * 3) + (X * 4)) %>% mutate (change2055 = Curr- Suit2055) %>% 
                mutate(across(Suit2055, round, 0)) %>% mutate(Suit2055 = sub('4', 'X', Suit2055))
  
  datMid2 <- datMid2 %>% mutate(Trajectory2055 = 
  ...
  datMid2 <- datMid2 %>% mutate(FailRisk2055 = 
                                  ifelse(X>.5, "High", 
                                         ifelse(X>.2 & X<.5, "Increased", "Normal")))
  ...
  datLong2 <- datLong
  datLong2 <- datLong2 %>% mutate(Suit2085 = `1` +(`2` * 2 ) + (`3` * 3) + (X * 4)) %>% mutate (change2085 = Curr- Suit2085) %>% 
              mutate(across(Suit2085, round, 0))%>% mutate(Suit2085 = sub('4', 'X', Suit2085))
  datLong2 <- datLong2 %>% mutate(Trajectory2085 = 
  ...
  datLong2 <- datLong2 %>% mutate(FailRisk2085 = 
                                  ifelse(X>.5, "High", 
                                         ifelse(X>.2 & X<.5, "Increased", "Normal")))
  
  datLong3 <- datLong2 %>% dplyr::select(SiteNo,SS_NoSpace,Spp,Curr, Suit2085, Trajectory2085, FailRisk2085)
  
  summOut <- full_join(datFeas_final, datEarly3, by = c('SiteNo','SS_NoSpace','Spp', 'Curr'))
   summOut <- full_join(summOut, datMid3, by = c('SiteNo','SS_NoSpace','Spp', 'Curr'))
 summOut <- full_join(summOut, datLong3, by = c('SiteNo','SS_NoSpace','Spp', 'Curr')) %>% filter(!Flag == 'NotIn')
